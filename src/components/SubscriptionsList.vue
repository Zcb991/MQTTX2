    ...
    <left-panel>
      ...
      >
        ...
        <div
          v-for="(sub, index) in subsList"
          :key="sub.topic"
          :class="['topics-item', { active: isActive(sub.topic), disabled: sub.disabled }]"
          :style="{
            background: `${sub.color}10`,
          }"
          @click="handleClickTopic(sub, sub.topic)"
          @contextmenu.prevent="handleContextMenu(sub, $event)"
        >
          <!-- <div></div> -->
          <div
          ...
          <!-- <span class="message-number">{{ topicCntObject[sub.topic] }}</span> -->
          <span class="qos">{{ topicCntObject[sub.topic] ? topicCntObject[sub.topic] : 0 }} QoS {{ sub.qos }}</span>
...
export default class SubscriptionsList extends Vue {
  ...
  @Prop({ required: true }) public activeArrays!: string[]
  @Prop({ required: true }) public topicCntObject!: object
  ...
  // private topicActiveArrays: string[] = []
  private topicActiveArrays: string[] = this.activeArrays
  ...
  private handleRecordChanged(val: ConnectionModel) {
    ...
    if (val.id) {
      ...
      console.log('subList:',this.subsList)
    }
  }

  @Watch('activeArrays')
  private handleActiveArraysChanged(val: string[]) {
    this.topicActiveArrays = val
  }
  ...
  private handleClickTopic(item: SubscriptionModel, topic: string) {
    // 左键单击主题时触发的事件
    if (item.disabled) {
      return
    }
    ...
    console.log('isActive(topic):')
    // console.log('isActive(topic):', this.isActive(topic))
    ...
    //   // 如果已经选中，则取消选中
    //   this.topicActiveArrays = this.topicActiveArrays.filter((selectedTopic : string) => selectedTopic !== topic)
    //   // this.activeArrays = this.activeArrays.filter((selectedTopic : string) => selectedTopic !== topic)
    //   this.$emit('onClickTopic', item, false)
    // } else {
    //   // 如果未选中，则添加选中主题
    //   this.topicActiveArrays.push(topic)
    //   // this.activeArrays.push(topic)
    //   this.$emit('onClickTopic', item, true)
    // }

  }

  private isActive(topic: string) {
    // 判断该主题是否已选中
    console.log('this.topicActiveArrays:', this.topicActiveArrays)
    console.log('this.topicActiveArrays.includes(topic):', this.topicActiveArrays.includes(topic))
    // return this.activeArrays.includes(topic)
    return this.topicActiveArrays.includes(topic)
  }
