<template>
  <div class="connections-detail">
    <!-- 控制右上方表单，根据vuex中的属性showConnections -> getterShowConnections来动态绑定样式 -->
    <!-- 112233 -->
    <!-- <div
    ...
    <div
      ref="connectionTopbar"
      class="connections-topbar right-topbar"
      :style="{ left: getterShowConnections ? '341px' : '81px' }"
    >
      ...
      <transition name="el-zoom-in-top">
        <div v-show="searchVisible" class="connections-search topbar">
          <el-input
            id="searchTopic"
            v-model="searchParams.searchTopic"
            size="small"
    ...
    <div
      ...
      :style="{
        ...
        marginLeft: getterShowConnections ? '591px' : '331px',
      }"
    >
      <div class="connections-body">
        ...
        <div
          ...
          :style="[
            ...
            { left: getterShowConnections ? '341px' : '81px' },
          ]"
        >
      ...
      <div
        ...
        :style="{ marginLeft: getterShowConnections ? '591px' : '331px' }"
      >
...
export default class ConnectionsDetail extends Vue {
  ...
  get topicCntObject(): object {
    // this.recordMsgs.topicCnt
    return this.recordMsgs.topicCnt
  }
  ...
  private updateMeta(message: MessageModel, optionName: 'function' | 'schema', optionMethod: 'received' | 'publish') {
    // console.log('this.checkScriptOption(optionName, optionMethod)', this.checkScriptOption(optionName, optionMethod))
  ...
  private async getMessages(limit = 20) {
    ...
    this.recordMsgs = await messageService.get(this.curConnectionId, {
      ...
      topics: this.activeArrays,
      searchParams: this.searchParams,
    })
    console.log('this.recordMsgs', this.recordMsgs)
  ...
  private async loadMoreMessages(mode: 'before' | 'after' = 'before') {
    ...
    console.log('loadMoreMessages loading')
  ...
  private handleTopicClick(sub: SubscriptionModel, reset: boolean) {
    ...
    // reset ? (this.activeTopic = '') : (this.activeTopic = sub.topic)
    console.log('父sub:', sub)
    console.log('sub是否为null:', sub === null)
    console.log('reset:', reset)

    // 点击主题的时候，会传递主题模型sub；
    // 程序加载完，只点击左边的连接列表，右边的主题列表从来没有点击过的时候，这时候传过来的sub是null

    // 如果reset为true，并且sub为null，则表明要重置激活的主题（清空数组）
    if (reset && (sub === null)) {
      console.log('reset activeArrays')
      this.activeArrays = []
    }

    // TODO: 这里的加载时机有点问题
    if (sub !== null) {
      if (this.activeArrays.includes(sub.topic)) {
        // 已经激活了，则移除topic
        console.log('topic移除中')
        this.activeArrays = this.activeArrays.filter((topic) => topic !== sub.topic)
        console.log('移除后：', this.activeArrays)
      } else {
        // 没有激活，则向数组中增加topic
        console.log('topic增加中')
        this.activeArrays.push(sub.topic)
        console.log('增加后：', this.activeArrays)
      }
    }
    this.handleMessages()
  }
  ...
  private printMessageLog(id: string, message: MessageModel) {
    try {
      ...
      if (id === this.curConnectionId) {
        // TODO:这里是当某个主题被点击的时候，如果有消息收发，则会进行日志的记录
        // 由于时间原因，这里先不改造了
    ...
    try {
      ...
      const pushMsgs = (msgs: MessageModel[]) => {
        ...
        msgs.forEach((msg: MessageModel) => {
          ...
          // console.log('isActiveTopicMessages', isActiveTopicMessages)
